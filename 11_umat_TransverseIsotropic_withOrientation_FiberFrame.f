...
C
      integer i, j
	  real :: x
!	  real, dimension(3) :: func
	  real, dimension(3) :: a,b
	  real, DIMENSION(3, 3) :: E
	  real*8, dimension( 3,3 ) :: iden,dF,dF_mid,h_mid,D_mid,W_mid,Q_mid
...
!	    inv_dfgrd1 = inv3(dfgrd1)
!      call inv3(dfgrd0,inv_dfgrd0)
      WRITE(*,*) ('inverse of dfgrd1 =') 
      WRITE(*,*) (inv_dfgrd1)  	
      
	  dF=matmul(dfgrd1,inv3(dfgrd0));
	  dF_mid=0.5d0*(dF+IDEN);
	  h_mid=1.0d0/0.5d0*(IDEN-inv3(dF_mid));
	  D_mid=0.5d0*(h_mid+transpose(h_mid));
	  W_mid=0.5d0*(h_mid-transpose(h_mid));
	  Q_mid = matmul(inv3(IDEN-0.5*W_mid),(IDEN+0.5*W_mid));
      WRITE(*,*) ('drot  =') 
      WRITE(*,*) (Q_mid)  	
! ======================================================
!	  b = (/1.0,2.0,3.0/)
...
C     Subroutine to inverse a 3*3 matrix    
!      INCLUDE 'inv3.for'
! 	  INCLUDE 'PolarDecomposition.for'
	  
	  INCLUDE 'linearalgebra.for'